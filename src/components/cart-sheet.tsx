...
export function CartSheet({ open, onOpenChange }: CartSheetProps) {
  ...
  const handleCheckout = () => {
    const COMPANY_WHATSAPP_NUMBER = "573104627014"; // Reemplaza con número real

    let message =
      "Hola, me gustaría hacer el siguiente pedido de la tienda Perfect Nails:\n\n";
    items.forEach((item) => {
      ...
      {open && (
        ...
        >
          ...
          <div className="absolute right-0 top-0 h-full w-full sm:w-[28rem] pointer-events-auto flex flex-col">
            <div className="absolute inset-0 glass-panel !rounded-r-none !border-r-0 border-l border-white/40 shadow-[-10px_0_30px_rgba(0,0,0,0.03)] sm:rounded-l-[2.5rem]" />
            ...
            <div className="relative h-full flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto p-6 no-scrollbar space-y-6">
                ...
                >
                  ...
                  <div className="size-8 rounded-full bg-[var(--quartz)] flex items-center justify-center group-hover:scale-110 transition-transform">
                    <ChevronDown
                      className={cn(
                        "size-4 text-[var(--gold)] transition-transform duration-500",
                    ...
                    {showItems && (
                      ...
                      >
                        {items.map((item) => (
                          <div
                            key={item.id}
                            className="glass-card !rounded-[2.2rem] p-4 sm:p-5 hover:shadow-xl transition-all relative overflow-hidden group"
                          >
                              ...
                              <div className="flex flex-1 flex-col justify-center">
                                ...
                                <div className="mt-6 flex items-center justify-between">
                                  <div className="flex items-center gap-3 bg-[var(--quartz)] rounded-full p-1 shadow-inner border border-black/5">
                                    ...
                                    <span className="text-xs sm:text-sm font-bold w-4 text-center text-[var(--ink)]">
                                      {item.quantity}
                                    </span>
                                    <button
                                      onClick={() =>
              ...
              {items.length > 0 && (
                <div className="glass-panel !rounded-t-none !rounded-br-none !border-b-0 !border-x-0 p-6 sm:p-8 pb-10 sm:pb-12 space-y-6 shadow-[0_-24px_48px_rgba(0,0,0,0.02)] shrink-0 sticky bottom-0 z-20 sm:rounded-bl-[2.5rem]">
                  <div className="space-y-2 sm:space-y-3">
                    ...
                    <div className="flex items-center justify-between text-xs sm:text-sm font-medium">
                      ...
                      {shipping === 0 ? (
                        <span className="text-emerald-600 font-bold uppercase text-[9px] sm:text-[10px] tracking-widest bg-emerald-50 px-2 py-0.5 rounded-full">
                          Gratis
                        </span>
                      ) : (
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[8px] sm:text-[9px] text-[var(--ink-soft)] font-bold uppercase tracking-widest">
                          Pago Final
                        </p>
                      </div>
                      <div className="text-right">
                        <span className="font-display text-2xl sm:text-3xl font-bold text-[var(--gold)]">
                          {formatCurrency(grandTotal)}
                        </span>
                        <p className="text-[9px] sm:text-[10px] text-[var(--ink-soft)] font-bold uppercase tracking-tighter">
                          IVA Incluido
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="grid gap-2 sm:gap-3">
                    <Button
                      ...
                      className="w-full h-12 sm:h-14 rounded-full text-base sm:text-lg font-bold shadow-xl shadow-[var(--gold-soft)]/20"
                    >
                      Confirmar y Pagar
